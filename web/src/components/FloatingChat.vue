...
const handleRoleChange = async (roleId: number) => {
  try {
    ...
    AMessage.success('Role switched successfully')
    ...
    AMessage.error('Failed to switch role')
...
const ensureConnection = async (): Promise<boolean> => {
  if (!isConnected.value) {
    try {
      ...
      if (!success) {
        AMessage.error('Not connected to server, please check chat configuration')
        return false
      }
      await new Promise(resolve => setTimeout(resolve, 300))
    } catch (error) {
      AMessage.error('Connection failed: ' + error)
      return false
    }
  }
  return true
...
const sendTextMessage = async () => {
  ...
  if (success) {
    ...
    AMessage.error('Send failed, please check connection status')
  }
}
...
const startRecording = async () => {
  ...
  try {
    ...
    AMessage.error('Unable to start recording, please check microphone permissions')
  }
}
...
const stopRecording = async () => {
  ...
  try {
    ...
    AMessage.error('Failed to stop recording')
  }
}
